...
**IndexAll 是一个专为 AI 设计的资源表。**

其核心价值：
1. **帮助 AI 理解用户拥有什么资源** — AI 可以查询 IndexAll 快速获得用户的资源全景
2. **高效为 AI 提供某个标签下的所有资源** — 支持快速检索特定分类的资源列表

**使用场景**：
- 用户问 Claude："我有哪些 Python 学习资源？"
  → Claude 调用 IndexAll API 查询 `tag:learning/python`
  → Claude 基于返回的资源列表给出智能化的回答和建议

**定位**：
- IndexAll **不是** UI-first 的笔记应用或书签工具
- IndexAll **是** API-first 的数据层，为各类 AI 系统提供统一的资源查询接口
- 用户通过简单的导入和管理界面维护这个"资源表"，AI 通过 API 高效查询和利用它
...
- ✅ 简洁高效的 API（为 AI 查询优化）
- ✅ 快速导入机制（用户维护资源的低成本）
- ❌ 不承诺自动聚合各系统资源（那是 Connector 的职责，Phase 2+）
...
当前市场上的产品主要集中在"书签管理"赛道，即管理网页链接。尚无产品真正实现跨系统资源的统一索引。

### 主要竞品

| 产品 | 类型 | 标签能力 | 跨系统联动 | 第三方插件 | 局限 |
|---|---|---|---|---|---|
| **Raindrop.io** | SaaS | 扁平标签 + 收藏集 | ✗ | 仅浏览器扩展 | 标签无层级；只管理网页链接，不索引其他系统资源 |
| **Karakeep** (原 Hoarder) | 开源自托管 | 扁平标签（AI 自动打标） | 仅 RSS + 浏览器书签导入 | 浏览器扩展 + 移动端 | 标签无层级无别名；无 Connector 体系，不能主动从第三方拉取 |
| **Anybox** | macOS 付费 | **嵌套标签** + 智能列表 | ✗ | ✗ | 仅 Apple 生态；无第三方系统联动 |
| **TagSpaces** | 开源 | 文件级标签 | ✗ | ✗ | 面向本地文件管理，不索引外部平台资源 |
| **Linkwarden** | 开源自托管 | 层级收藏集 | ✗ | 浏览器扩展 | 侧重网页存档；无标签 DAG，无跨系统能力 |
| **Faved** | 开源自托管 | 嵌套标签 | ✗ | ✗ | 纯书签管理，功能较基础 |

### IndexAll 的差异化定位

| 维度 | 竞品现状 | IndexAll |
|---|---|---|
| **资源范围** | 仅管理网页链接/书签 | 索引所有系统的资源（网页、GitHub repo、Notion 页面、本地文件等） |
| **标签结构** | 扁平标签或单一树形层级 | DAG 网状结构（一个标签多个父级） + 别名 |
| **跨系统联动** | 无或仅支持导入 | Connector 体系，主动采集 + 同步 + 追踪变更 |
| **第三方深度集成** | 仅浏览器扩展 | 计划开发 Obsidian / VS Code / Raycast 等插件，在各软件内直接操作 IndexAll |

**核心差异**：竞品是"书签管理器"，IndexAll 是"跨系统资源索引器"。竞品管理的是链接副本，IndexAll 管理的是指向各系统资源的活指针，并通过 Connector 保持映射关系。
